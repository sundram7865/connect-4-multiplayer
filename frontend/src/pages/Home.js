...
function Home({ socket }) {
    ...
    useEffect(() => {
        if (
            (location.pathname === "/ai" && (!OKClick && level !== null) && buttonClicked) ||
    ...
    }, [backendUrl, navigate, setUsername, setAvatar])
    ...
    useEffect(() => {
        if (location.pathname === "/home") {
            ...
            axios.get(`${backendUrl}/ai/clear-data`, { withCredentials: true }).catch(error => {
                console.log(error)
            })
            axios.get(`${backendUrl}/multiplayer/clear-data`, { withCredentials: true }).catch(error => {
                console.log(error)
            })
        }
    }, [
        ...
        setPlayers, setTime, setOKClick, setFirstPlayerClick, backendUrl
    ])
